...
CT.ReplaceButtons = function() {
	function HtmlToJA(id,text) {
		return document.getElementById(id).innerHTML = text;
	}
	HtmlToJA('prefsButton', 'オプション')
	HtmlToJA('statsButton', '統計')
	HtmlToJA('logButton', '情報')

};
/*ゲームのループをフック*/
CT.Backup.Loop = Game.Loop;
Game.Loop = function() {
	CT.Backup.Loop();
	CT.Render.Legacyway();
}
